...
def overview_summary():
    ...
    try:
        ...
        prompt = (
            ...
            f'- 可以带 emoji，但不要满屏都是；标签必须同时有文字和 emoji，emoji 只放 1 个且在文字最前面。\n'
            ...
            f'【mood 选择指南】根据数据综合判断，不要只看碎片化：\n'
            f'- 工作时长>=6h 且深度占比>=40% → 优先 focused，不要选 scattered\n'
            f'- 工作时长>=6h 且有明确主力应用 → 优先 proud 或 balanced\n'
            f'- 碎片化>70 且深度占比<25% 且切换>50次 → 才考虑 scattered\n'
            f'- 工作时长<3h → 优先 warm 或 excited\n'
            f'- 深夜工作到很晚 → 优先 tired\n'
            f'注意：工作时长长本身就是努力的体现，不要因为切换多就否定一天的成果。尽量避免连续选 scattered。\n'
