  ...
  return (
    <div className="animate-fade-in space-y-5">
      ...
      {range === 'week' ? (
        ...
        <div className="card">
          {heatmapError ? (
            <ApiErrorDisplay error={new Error(heatmapError)} onRetry={() => setBaseDate((d) => d)} />
          ) : heatmapLoading ? (
            <div className="text-center py-8 text-cd-text-tertiary animate-pulse">加载中...</div>
            ...
            <>
              <h2 className="text-sm font-semibold text-cd-text mb-3">
                {baseDate.format('YYYY')} 年专注热力图（点击查看当日时间线）
              </h2>
              {/* GitHub 风格：53 列 × 7 行 */}
              <div
                className="grid gap-1 overflow-x-auto"
                style={{ gridTemplateRows: 'repeat(7, 1fr)', gridAutoFlow: 'column', width: 'fit-content', maxWidth: '100%' }}
              >
                {monthYearCells.map((cell) => (
                  <div
                    key={cell.date}
                    className="w-3 h-3 rounded-sm cursor-pointer hover:ring-1 hover:ring-cd-purple"
                    style={{ backgroundColor: cellColor(cell.level) }}
                    title={`${cell.date}: ${cell.focus_min} 分钟`}
                    onClick={() => navigate(`/timeline?date=${cell.date}`)}
                  />
                ))}
              </div>
              <div className="mt-3 text-xs text-cd-text-tertiary">
                提示：颜色越深表示当日专注时间越长，点击格子跳转到对应日时间线。
              </div>
            </>
          ) : (
            <>
              ...
              <div className="grid grid-cols-7 gap-1.5">
                ...
                {monthYearCells.map((cell) => (
                  <div
                    key={cell.date}
                    className="aspect-square rounded-sm cursor-pointer hover:ring-1 hover:ring-cd-purple flex items-center justify-center text-[9px]"
                    style={{ backgroundColor: cellColor(cell.level) }}
                    title={`${cell.date}: ${cell.focus_min} 分钟`}
                    onClick={() => navigate(`/timeline?date=${cell.date}`)}
                  >
