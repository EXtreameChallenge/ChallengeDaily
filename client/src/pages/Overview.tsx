  ...
  return (
    <div className="animate-fade-in space-y-3">
      ...
      <div className="grid grid-cols-6 gap-3">
        {[
          { label: '记录条数', value: activities.length > 0 ? `${Math.max(activities.length, captureCount)}` : `${captureCount}`, icon: ActivityIcon, color: 'text-cd-green' },
          { label: '专注时长', value: timeStr, icon: Clock, color: 'text-cd-green' },
          { label: '深度工作', value: `${focusSessions}次`, icon: Focus, color: 'text-cd-blue' },
          { label: '截图次数', value: `${captureCount}`, icon: Camera, color: 'text-cd-purple' },
          { label: '主要分类', value: topCategory, icon: Monitor, color: CATEGORY_COLORS[topCategory] ? '' : 'text-cd-orange', customColor: CATEGORY_COLORS[topCategory] },
          { label: '高效时段', value: peakPeriod ? rhythmShort[peakPeriod] || peakPeriod.split(' ')[0] : '-', icon: TrendingUp, color: 'text-cd-orange' },
        ].map(({ label, value, icon: Icon, color, customColor }) => (
          <div key={label} className="bg-cd-card border border-cd-border rounded-xl px-4 py-4 text-center">
            <Icon size={18} className={`mx-auto mb-2 ${color}`} style={customColor ? { color: customColor } : undefined} />
            <div className="text-xl font-bold text-cd-text truncate">{value}</div>
            <div className="text-xs text-cd-text-tertiary mt-1">{label}</div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-5 gap-3">
        ...
        <div className="col-span-2 space-y-3">
          ...
          <div className="bg-cd-card border border-cd-border rounded-xl p-5">
            <h3 className="text-sm font-semibold text-cd-text mb-3">时间都去哪了</h3>
            {categoryBars.length > 0 ? (
                  ...
                  return (
                    <div key={entry.name}>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-xs text-cd-text-tertiary shrink-0 ml-2">{formatDur(entry.value)} · {pct}%</span>
                      </div>
              ...
              <p className="text-sm text-cd-text-tertiary text-center py-4">暂无分类数据</p>
            )}
          </div>

          {/* 应用使用 */}
          <div className="bg-cd-card border border-cd-border rounded-xl p-5">
            <h3 className="text-sm font-semibold text-cd-text mb-3">应用使用 Top 5</h3>
            {topApps.length > 0 ? (
              <div className="space-y-3">
                {topApps.map((app) => {
                  ...
                  return (
                    <div key={app.app_name} className="flex items-center gap-2.5">
                      <div className="w-6 h-6 rounded-md bg-cd-bg-secondary flex items-center justify-center text-xs font-medium text-cd-text-secondary shrink-0">{iconChar}</div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-sm text-cd-text truncate">{app.app_name}</span>
                          <span className="text-xs text-cd-text-tertiary shrink-0 ml-1">{formatDur(app.duration_min)}</span>
                        </div>
                        <div className="h-2 rounded-full bg-cd-bg-secondary overflow-hidden">
              ...
              <p className="text-sm text-cd-text-tertiary text-center py-4">暂无应用记录</p>
            )}
          </div>
        </div>
        ...
        <div className="col-span-3 space-y-3">
          {/* 3天热力图 */}
          <div className="bg-cd-card border border-cd-border rounded-xl p-5">
            <h3 className="text-sm font-semibold text-cd-text mb-3">近三天活动热力图</h3>
          ...
          {/* 7天趋势 */}
          <div className="bg-cd-card border border-cd-border rounded-xl p-5">
            <h3 className="text-sm font-semibold text-cd-text mb-3">效率趋势</h3>
            {validTrend.length > 0 ? (
              ...
              <p className="text-sm text-cd-text-tertiary text-center py-6">数据积累中，明天开始可看趋势</p>
            )}
          </div>
        </div>
      </div>
        ...
        <div className="bg-cd-card border border-cd-border rounded-xl p-5">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {recentActivities.map((act) => {
              ...
              return (
                <div key={act.id}
                  className="shrink-0 bg-cd-bg-secondary rounded-lg px-3 py-2 min-w-0 max-w-[220px] cursor-default hover:bg-cd-hover transition"
      ...
      {rhythmData.length > 0 && (
        <div className="bg-cd-card border border-cd-border rounded-xl p-5">
          <h3 className="text-sm font-semibold text-cd-text mb-3">
            个人节奏{peakPeriod && <span className="font-normal text-cd-text-tertiary ml-2">你在<span className="text-cd-green font-medium ml-0.5">{peakPeriod}</span>效率最高</span>}
          </h3>
          <div className="grid grid-cols-4 gap-4">
            {rhythmData.map((item) => {
              const color = item.duration_min === Math.max(...rhythmData.map(r => r.duration_min)) ? 'var(--cd-green)' : 'var(--cd-text-tertiary)'
              const barColor = item.duration_min === Math.max(...rhythmData.map(r => r.duration_min)) ? 'bg-cd-green' : 'bg-cd-green/40'
              return (
                <div key={item.period}>
                  ...
                  <div className="text-[11px] text-cd-text-tertiary mt-1">{item.percentage}%</div>
                </div>
              )
            })}
          </div>
